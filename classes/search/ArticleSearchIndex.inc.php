...
class ArticleSearchIndex {
	...
	function &filterKeywords($text, $allowWildcards = false) {
		...
		// Remove punctuation
		$cleanText = OjsString::regexp_replace('/[!"\#\$%\'\(\)\.\?@\[\]\^`\{\}~]/', '', $cleanText);
		$cleanText = OjsString::regexp_replace('/[\+,:;&\/<=>\|\\\]/', ' ', $cleanText);
		$cleanText = OjsString::regexp_replace('/[\*]/', $allowWildcards ? '%' : ' ', $cleanText);
		$cleanText = OjsString::strtolower($cleanText);

		// Split into words
		$words = OjsString::regexp_split('/\s+/', $cleanText);
		...
		$keywords = array();
		foreach ($words as $k) {
			if (!isset($stopwords[$k]) && OjsString::strlen($k) >= $minLength && !is_numeric($k)) {
				$keywords[] = OjsString::substr($k, 0, SEARCH_KEYWORD_MAX_LENGTH);
			}
		}
		return $keywords;
	}
