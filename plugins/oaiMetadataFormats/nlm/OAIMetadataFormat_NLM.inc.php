...
class OAIMetadataFormat_NLM extends OAIMetadataFormat {
	...
	function toXml($record, $format = null) {
		...
		$matches = null;
		if (OjsString::regexp_match_get('/^[Pp][Pp]?[.]?[ ]?(\d+)$/', $article->getPages(), $matches)) {
			$matchedPage = htmlspecialchars(Core::cleanVar($matches[1]));
			$response .= "\t\t\t\t<fpage>$matchedPage</fpage><lpage>$matchedPage</lpage>\n";
			$pageCount = 1;
		} elseif (OjsString::regexp_match_get('/^[Pp][Pp]?[.]?[ ]?(\d+)[ ]?-[ ]?([Pp][Pp]?[.]?[ ]?)?(\d+)$/', $article->getPages(), $matches)) {
		...
		if (!empty($abstract)) {
			...
			// $abstract = '<p>' . OjsString::regexp_replace('/\n+/', '</p><p>', $abstract) . '</p>';
